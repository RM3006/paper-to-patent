...
def corpus_db() -> Generator[duckdb.DuckDBPyConnection, None, None]:
    ...
    In production dim_paper/dim_patent are exclusion-filtered upstream (by
    document_exclusions), so load_corpus should never meet a version-style or
    non-English row. The fixture includes a few anyway (W004, W005, P002, P003)
    to confirm load_corpus skips them defensively rather than crashing.
    """
    con = duckdb.connect()
...
def test_load_corpus_skips_patent_with_no_title(
    corpus_db: duckdb.DuckDBPyConnection,
) -> None:
    # A patent with no usable title is skipped from the embeddable corpus.
    # The authoritative exclusion is recorded upstream by document_exclusions,
    # not here; load_corpus just skips it defensively.
    corpus = load_corpus(corpus_db)
    assert all(d["doc_id"] != "P002" for d in corpus)
...
def test_load_corpus_skips_defensively_filtered_rows(
    corpus_db: duckdb.DuckDBPyConnection,
) -> None:
    # In production these never reach dim_paper/dim_patent (document_exclusions
    # removes them upstream); if one somehow does, load_corpus skips it rather
    # than crashing: W004 (non-English), W005 & P003 (version-style titles).
    corpus = load_corpus(corpus_db)
    skipped = {"W004", "W005", "P003"}
    assert all(d["doc_id"] not in skipped for d in corpus)
