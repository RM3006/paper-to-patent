...
2. **Seed crosswalk** — a small hand-maintained map for the unambiguous heavyweights in scope, e.g. NVIDIA / TSMC / ASML / Stanford / IMEC, `match_method = seed_crosswalk`. Covers the head of the distribution before fuzzy matching.
3. **Fuzzy bridge** — block on a cheap key, then `rapidfuzz` token-set ratio; auto-accept above a precision threshold (`fuzzy_high`) and route the band below to `fuzzy_review`. `splink` is escalated to only if rapidfuzz precision on the eval set falls below 0.95.
Quality is measured against a hand-labelled eval set (`docs/er_eval_set.md`), drawn from orgs that appear in **both** sources within the scope, favouring **precision over recall**.

**Considered.** Exact match after normalisation only; `splink` as the primary fuzzy engine; embedding-based name similarity; a commercial entity-resolution API; a purely manual crosswalk.

**Why.** The two sources share no key and the cross-source bridge is the project's hard problem. Exact-match-after-cleaning is brittle ("NVIDIA" vs "Nvidia Corporation"). `rapidfuzz` token-set ratio handles most of the long tail cleanly, is fast, and has one dependency. `splink`'s Fellegi-Sunter model adds calibration value when multiple discriminating fields are available; with one strong field (normalised name), rapidfuzz usually suffices — escalation is data-driven, not assumed. Precision is favoured over recall because a single false merge poisons every downstream competitive-intelligence number. Person-level matching (paper authors ↔ patent inventors) is explicitly **out of scope for v1**: name disambiguation across these sources is a separate hard project, deferred to v2.
<!-- /MAINTAINED -->
