...
Quality is measured against a hand-labelled eval set (`docs/er_eval_set.md`). Verified 2026-06-22 (score=100 rule): all 10 Tier-3 non-match pairs correctly excluded; precision = 1.00. Crosswalk size verified directly against live MotherDuck prod, 2026-07-14: 16,235 rows / 14,198 distinct `org_id`s (ror 11,763 · native_id 2,559 · fuzzy_high 1,818 · seed_crosswalk 57 · ror_bridge 38).
...
**Why.** The served data is small and read-only, and MotherDuck is the same DuckDB engine the marts were built with, so the app queries it directly — the fewest hops, the smoothest demo, and no export step to keep in sync. Streamlit is the fastest path to a hosted public demo a recruiter can open in one click. Plotly `scattergl` (WebGL) is required to render tens of thousands of points smoothly. A FastAPI tier is noted as a v2 hardening option, not a v1 need; Evidence.dev is a reasonable future alternative for a more BI-as-code feel. Least privilege calls for a read-only MotherDuck token on the app, but MotherDuck's free tier cannot issue read-scaling (read-only) tokens — the app runs publicly (deployed 2026-07-14 at https://paper-to-patent-a7iiegantbeucyxxwegpyz.streamlit.app/) on the same read-write token as the build pipeline. Accepted risk, made knowingly rather than by default: the warehouse is fully derived from R2 and rebuildable via `dbt build --target prod` in ~1 min, so a leak means downtime, not data loss. Move to a genuine read-only token on tier upgrade.
...
**Intermediate**
- `int_org_crosswalk` — the rapidfuzz-built `org_id` mapping (see §6).
...
- `mart_competitive` — grain (cluster, side, org, **document-level family**): assignees capturing IP vs institutions producing research, with counts and shares. Widened 2026-07-12 to carry each row's own `family_id`/`family_id_key` (5-way, from the facts, `coalesce`d to `'unattributed'` — never NULL) alongside the existing cluster grain, so a family filter and a cluster filter can combine on the same mart. `share`/`cluster_total` keep their original whole-cluster (all-families) meaning; `rank_in_cluster` is the org's rank by its TOTAL doc_count across all its family slices, not a per-family rank.
...
- `mart_family` — one row per one of the **5 document-level families** (rebuilt 2026-07-12; see two-tier tagging above): papers, patents, patent share, exact research-org/assignee breadth, a TRUE median citation lag (not a weighted average), and NPL link count. Aggregates `fact_patent_filing`/`fact_publication` directly by their own `family_id` — the same authoritative column the two-tier note above describes — not `seed_cluster_family`. Documents with no resolvable `family_id` are not a row here; the UI discloses that count separately rather than folding it into one of the 5.
<!-- /MAINTAINED -->
...
- **App runs on a read-write MotherDuck token, not read-only (free-tier constraint).** MotherDuck's free tier cannot issue read-scaling (read-only) tokens, so the public app shares the build pipeline's read-write token — a departure from least privilege, accepted knowingly rather than by default. The warehouse is fully derived and rebuildable from R2 in about a minute (`dbt build --target prod`), so a leaked token risks downtime, not data loss. Upgrading the MotherDuck tier and issuing a genuine read-only token is a v2 item.
